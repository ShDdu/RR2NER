...
class CoPredictor(nn.Module):
    def __init__(self, cls_num, hid_size, biaffine_size, dropout=0.0):
        super().__init__()
    ...
    def forward(self, x, y, position, cln):
...
class ffm(nn.Module):
    def __init__(self, config):
        ...
        self.model_dropout = config.model_dropout
        self.ffm_out = config.ffm_out
        self.conv_input_size = config.dist_emb_size + config.type_emb_size
        ...
        self.position = MLP(self.conv_input_size, self.ffm_out, act="relu", dropout=self.model_dropout)
        self.predictor = CoPredictor(self.ffm_out, self.lstm_hid_size, self.biaffine_size, self.model_dropout)  # B, C, H, W
        self.cln = FfmLayerNorm(self.lstm_hid_size, self.lstm_hid_size, conditional=True)
        self.cln_mlp = MLP(self.lstm_hid_size, self.ffm_out, act="relu", dropout=self.model_dropout)
    ...
    def forward(self, word_reps, word_length, dist_inputs, loss_mask):
        ...
        cln = self.cln_mlp(cln)
        dis_emb = self.dis_embs(dist_inputs)
        tril_mask = torch.tril(loss_mask.clone().long())
        reg_inputs = tril_mask + loss_mask.clone().long()
        reg_emb = self.reg_embs(reg_inputs)

        conv_inputs = torch.cat([dis_emb, reg_emb], dim=-1)
        position = self.position(conv_inputs)
        outputs = self.predictor(word_reps, word_reps, position, cln)
...
class rdm(nn.Module):
    def __init__(self, config, num_labels):
        ...
        self.cnn_1 = ComCNN(input_channels=self.ffm_out, output_channels=self.yolo_channel, kernels=(7, 7), stride=1, padding=3, bn=True, act="leakyrelu")
...
class RR2NER(nn.Module):
    def __init__(self, config):
        super(RR2NER, self).__init__()
        self.num_labels = len(config.label_list)
        self.embedding = EmbeddingLayer(config)
        self.ffm = ffm(config)
        self.rdm = rdm(config, self.num_labels)
    ...
    def forward(self, input_ids, word_ids=None, char_ids=None, token_masks_char=None, char_count=None, word_length=None, pieces_index=None, pieces_length=None,
        ...
        ffm_output = self.ffm(word_embedding, word_length, dist_inputs, loss_mask)
        rdm_outputs, rdm_pred = self.rdm(ffm_output)

        loss = None
        if labels is not None:
            loss = self.compute_loss(rdm_pred, labels)
        return {"output": rdm_outputs, "loss": loss}

    @staticmethod
    def compute_loss(yolo_pred, labels):
        mse_loss = nn.MSELoss()
        bce_loss = nn.BCELoss()
        obj_scale, noobj_scale = 1, 10
